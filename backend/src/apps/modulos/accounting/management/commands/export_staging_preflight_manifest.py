...
from apps.modulos.accounting.staging_ops import build_staging_ops_evidence, collect_staging_preflight_manifest


class Command(BaseCommand):
    help = "Genera preflight unificado staging-first (F6 + F7A + F7B) con umbrales de gate."
    ...
    def handle(self, *args, **options):
        ...
        try:
            payload = collect_staging_preflight_manifest(
        ...
        if output:
            ...
            self.stdout.write(self.style.SUCCESS(f"staging preflight exported: {path}"))
        else:
            self.stdout.write(raw)

        if strict and not bool(payload.get("preflight_passed")):
            raise CommandError("staging preflight gate failed.")
