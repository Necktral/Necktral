...
def custom_exception_handler(exc, context):
    ...
    data_scope = getattr(request, "data_scope", None) if request else None
    if data_scope:
        metadata["data_scope"] = data_scope

    intercompany = getattr(request, "intercompany", None) if request else None
    if intercompany:
        metadata["intercompany"] = intercompany

    ctx = getattr(request, "ctx", None) if request else None
    if ctx is not None:
        metadata["ctx"] = {
