# RBAC: por defecto scoped-only para reducir riesgo de privilegios globales legacy.
# Dev/test pueden sobreescribir a True durante transición controlada.
...
from config.settings.security import MIN_HS256_KEY_BYTES
...
# Clave de firma JWT: por defecto usa SECRET_KEY para mantener compatibilidad.
# En producción se recomienda definirla explícitamente y con >= 32 bytes.
JWT_SIGNING_KEY = env("DJANGO_JWT_SIGNING_KEY", default=SECRET_KEY)
...
# Exponer umbral para entornos que validan fortaleza de claves (ej. prod.py)
JWT_MIN_SIGNING_KEY_BYTES = MIN_HS256_KEY_BYTES
