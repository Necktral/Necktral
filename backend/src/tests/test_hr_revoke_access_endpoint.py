...
def test_hr_revoke_access_deactivates_memberships_and_position_roles_and_audits():
    ...
    # Call endpoint
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "admin_revoke", "password": "pass12345"}, format="json")
...
def test_hr_revoke_access_409_when_no_linked_user():
    ...
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "admin_revoke2", "password": "pass12345"}, format="json")
    assert login.status_code == 200
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
