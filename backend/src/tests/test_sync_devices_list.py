...
def test_sync_devices_list_returns_devices_for_company():
    ...
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "owner_list", "password": "pass12345"}, format="json")
...
def test_sync_devices_list_denied_without_permission_is_audited():
    ...
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "owner_no_list", "password": "pass12345"}, format="json")
    assert login.status_code == 200
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
