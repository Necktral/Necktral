...
def test_end_assignment_endpoint_deactivates_assignment_and_position_roles():
    ...
    def test_employee_list_includes_active_assignment_summary():
        ...
        client = APIClient()
        login = client.post("/api/backend/auth/login/", {"username": "admin_list_emp", "password": "pass12345"}, format="json")
    ...
    def test_list_employee_assignments_endpoint_returns_rows():
        ...
        client = APIClient()
        login = client.post("/api/backend/auth/login/", {"username": "admin_list_asg", "password": "pass12345"}, format="json")
        assert login.status_code == 200
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
    ...
    # Call endpoint
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "admin_end", "password": "pass12345"}, format="json")
    assert login.status_code == 200
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
...
def test_hr_reset_temp_password_success_and_audit():
    ...
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "admin_reset", "password": "pass12345"}, format="json")
    assert login.status_code == 200
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
...
def test_hr_reset_temp_password_409_when_no_linked_user():
    ...
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "admin_reset2", "password": "pass12345"}, format="json")
    assert login.status_code == 200
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {login.data['access']}")
