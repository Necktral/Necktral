...
def test_login_success_creates_contractual_audit_event():
    ...
    client = APIClient()
    r = client.post("/api/backend/auth/login/", {"username": "u1", "password": "pass12345"}, format="json")
...
def test_login_failed_creates_contractual_audit_event():
    ...
    client = APIClient()
    r = client.post("/api/backend/auth/login/", {"username": "u2", "password": "bad"}, format="json")
...
def test_logout_requires_auth_and_creates_audit_event():
    ...
    client = APIClient()
    r = client.post("/api/backend/auth/login/", {"username": "u3", "password": "pass12345"}, format="json")
    assert r.status_code == 200
    refresh = r.data["refresh"]
    access = r.data["access"]

    # 1) sin Authorization -> 401 (bloqueado por IsAuthenticated)
    r2 = client.post("/api/backend/auth/logout/", {"refresh": refresh}, format="json")
    assert r2.status_code == 401

    # 2) con Authorization -> 204 + audit AUTH_LOGOUT
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {access}")
    client.defaults["HTTP_AUTHORIZATION"] = f"Bearer {access}"
    r3 = client.post("/api/backend/auth/logout/", {"refresh": refresh}, format="json")
...
def test_logout_is_idempotent_when_refresh_missing():
    ...
    client = APIClient()
    r = client.post("/api/backend/auth/login/", {"username": "u4", "password": "pass12345"}, format="json")
    assert r.status_code == 200
    access = r.data["access"]

    client.credentials(HTTP_AUTHORIZATION=f"Bearer {access}")
    client.defaults["HTTP_AUTHORIZATION"] = f"Bearer {access}"
    r2 = client.post("/api/backend/auth/logout/", {}, format="json")
...
def test_logout_is_idempotent_when_refresh_invalid():
    ...
    client = APIClient()
    r = client.post("/api/backend/auth/login/", {"username": "u5", "password": "pass12345"}, format="json")
    assert r.status_code == 200
    access = r.data["access"]

    client.credentials(HTTP_AUTHORIZATION=f"Bearer {access}")
    client.defaults["HTTP_AUTHORIZATION"] = f"Bearer {access}"
    r2 = client.post("/api/backend/auth/logout/", {"refresh": "not-a-jwt"}, format="json")
    assert r2.status_code == 204
...
def test_logout_is_idempotent_when_refresh_belongs_to_another_user():
    ...
    client_a = APIClient()
    r_a = client_a.post("/api/backend/auth/login/", {"username": "uA", "password": "pass12345"}, format="json")
    assert r_a.status_code == 200
    refresh_a = r_a.data["refresh"]

    client_b = APIClient()
    r_b = client_b.post("/api/backend/auth/login/", {"username": "uB", "password": "pass12345"}, format="json")
    ...
    r2 = client_b.post("/api/backend/auth/logout/", {"refresh": refresh_a}, format="json")
