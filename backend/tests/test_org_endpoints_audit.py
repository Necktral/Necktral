...
def test_org_create_branch_creates_profile_and_audit():
    ...
    r = client.post("/api/backend/org/branches/", payload, format="json", HTTP_X_COMPANY_ID=str(company.id))
...
def test_org_patch_branch_updates_and_audit():
    ...
    r = client.patch(
        f"/api/backend/org/branches/{branch.id}/",
    ...
    ev = AuditEvent.objects.filter(
        ...
        module="ORG",
        path=f"/api/backend/org/branches/{branch.id}/",
        method="PATCH",
        subject_type="BRANCH",
        subject_id=str(branch.id),
    ).latest("timestamp_server")
...
def test_org_get_company_profile_requires_read_not_update():
    ...
    r = client.get("/api/backend/org/company/profile/", HTTP_X_COMPANY_ID=str(company.id))
    assert r.status_code == 200
...
def test_org_put_company_profile_updates_and_audit():
    ...
    r = client.put("/api/backend/org/company/profile/", payload, format="json", HTTP_X_COMPANY_ID=str(company.id))
    assert r.status_code == 200
    assert r.data["ok"] is True
    ...
    ev = AuditEvent.objects.filter(
        ...
        module="ORG",
        path="/api/backend/org/company/profile/",
        method="PUT",
...
def test_org_list_companies_and_create_company_audit():
    ...
    r0 = client.get("/api/backend/org/companies/", HTTP_X_COMPANY_ID=str(company1.id))
    ...
    r1 = client.post("/api/backend/org/companies/", payload, format="json", HTTP_X_COMPANY_ID=str(company1.id))
    ...
    ev = AuditEvent.objects.filter(
        ...
        module="ORG",
        path="/api/backend/org/companies/",
        method="POST",
        subject_type="COMPANY",
