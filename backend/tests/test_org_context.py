...
def test_context_requires_company_header_and_denies_without_membership():
    ...
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "u_ctx", "password": "pass12345"}, format="json")
    ...
    # Falta header -> 400
    r = client.get("/api/backend/iam/context/")
    assert r.status_code == 400

    # Company no accesible -> 403 + audit con required_scope
    r2 = client.get("/api/backend/iam/context/", HTTP_X_COMPANY_ID=str(c2.id))
    assert r2.status_code == 403

    ev = AuditEvent.objects.filter(event_type="AUTH_ACCESS_DENIED", path="/api/backend/iam/context/").latest("timestamp_server")
...
def test_context_allows_branch_membership_with_headers():
    ...
    client = APIClient()
    login = client.post("/api/backend/auth/login/", {"username": "u_ok", "password": "pass12345"}, format="json")
    assert login.status_code == 200
    access = login.data.get("access") if isinstance(login.data, dict) else None
    if isinstance(access, str) and access.count(".") == 2:
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {access}")
    ...
    r = client.get("/api/backend/iam/context/", HTTP_X_COMPANY_ID=str(c1.id), HTTP_X_BRANCH_ID=str(b1.id))
