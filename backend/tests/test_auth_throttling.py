...
def test_login_throttling_enveloped():
    ...
    with override_settings(REST_FRAMEWORK=override):
        ...
        try:
            ...
            r1 = client.post("/api/backend/auth/login/", {"username": user.username, "password": "pass12345"}, format="json")
            assert r1.status_code == 200

            r2 = client.post("/api/backend/auth/login/", {"username": user.username, "password": "pass12345"}, format="json")
...
def test_refresh_throttling_enveloped():
    ...
    with override_settings(REST_FRAMEWORK=override):
        ...
        try:
            ...
            login = client.post("/api/backend/auth/login/", {"username": user.username, "password": "pass12345"}, format="json")
            assert login.status_code == 200

            refresh = login.data.get("refresh")
            assert refresh

            r1 = client.post("/api/backend/auth/refresh/", {"refresh": refresh}, format="json")
            assert r1.status_code == 200

            refresh2 = r1.data.get("refresh") or refresh
            r2 = client.post("/api/backend/auth/refresh/", {"refresh": refresh2}, format="json")
            assert r2.status_code == 429
            payload = r2.json()
            assert payload["error"]["code"] == "RATE_LIMITED"
            assert payload["error"]["http_status"] == 429
...
def test_logout_throttling_enveloped():
    ...
    with override_settings(REST_FRAMEWORK=override):
        ...
        try:
            ...
            r1 = client.post("/api/backend/auth/logout/", {"refresh": refresh}, format="json")
